...
class PolynomialRegression:
    ...
    def generate_polynomial_features(self, X):
        ...
        #print(Phi)
        #one_s=
        Phi=np.array([np.zeros(1)+1]*n)
        #print(Phi)
        Phi=np.concatenate((Phi,X),axis=1)        
        #print(Phi)
        
        # the following code is for parts g, h, and i
    ...
    def fit_GD(self, X, y, eta=None, eps=0, tmax=10000, verbose=False):
        ...
        for t in range(tmax):
            ...
            if eta_input is None:
                #change this line
                #eta = 0.01  # before part (f)
                eta = 1/(1+t)  # after part (f)
            else:
                eta = eta_input
            ### ========== TODO : END ========== ###
    ...
    def cost(self, X, y):
        ...
        cost = np.sum(np.square(np.dot(X,self.coef_)-y))
        ### ========== TODO : END ========== ###
        return cost
...
def main():
    ...
    eta_s = [0.0001, 0.001, 0.01, 0.0407]
    for eta in eta_s:
        start = time.time()
        model = model.fit_GD(train_data.X,train_data.y,eta=eta)
        end = time.time()
        print ("eta=", eta, ", time=", end-start, ", cost=", model.cost(train_data.X,train_data.y), ", coef=", model.coef_)
        
    start = time.time()
    model = model.fit(train_data.X,train_data.y)
    end = time.time()
    print("Closed form, time=", end-start, "cost=", model.cost(train_data.X,train_data.y), "coef=", model.coef_)
        
    model = PolynomialRegression()
    start = time.time()
    model.fit_GD(train_data.X,train_data.y)
    end = time.time()
    print("(f)Linear regression with GD with changing eta, time=", end-start, "cost=", model.cost(train_data.X,train_data.y), "coef=", model.coef_)
    ...
    for i in range(11):
        ...
        y_train.append(model.rms_error(train_data.X,train_data.y))
        y_test.append(model.rms_error(test_data.X, test_data.y))
        
    x_axis = np.arange(11)
    plt.plot(x_axis,y_train,label="Training Error")
    plt.plot(x_axis,y_test,label="Test Error")
    plt.title("RMSE versus m")
    plt.xlabel("m")
    plt.ylabel("RMSE")
    plt.legend()
    plt.show()
